import logging

contacts = {}

logging.basicConfig(filename='contact.log', level=logging.INFO, format='%(asctime)s %(message)s')

while True:
    print('\nContact Book App')
    print('1. Create Contact')
    print('2. View Contact')
    print('3. Update Contact')
    print('4. Delete Contact')
    print('5. Search Contact')
    print('6. Count Contact')
    print('7. Exit')
    choice = input("Enter your choice : ")

    if choice == '1':
        name = input("Enter your name : ")
        if name in contacts:
            print(f"Contact name {name} already exists!")
            logging.warning(f"\nAttempted to create duplicate contact:\n Name: {name}\n\n{'-'*60}")
        else:
            age = input("Enter age : ")
            email = input("Enter email : ")
            mobile = input("Enter mobile : ")
            contacts[name] = {'age': int(age), 'email': email, 'mobile': mobile}
            print(f"Contact name {name} has been created successfully!")
            logging.info(f"\nContact created:\n Name: {name}\n Age: {age}\n Email: {email}\n Mobile: {mobile}\n\n{'-'*60}")

    elif choice == '2':
        name = input("Enter contact name to view : ")
        if name in contacts:
            contact = contacts[name]
            print(f"Name: {name}, Age: {contact['age']}, Mobile Number: {contact['mobile']}, Email: {contact['email']}")
            logging.info(f"\nViewed contact:\n Name: {name}\n Age: {contact['age']}\n Mobile: {contact['mobile']}\n Email: {contact['email']}\n\n{'-'*60}")
        else:
            print('Contact not found!')
            logging.warning(f"\nAttempted to view non-existent contact:\n Name: {name}\n\n{'-'*60}")

    elif choice == '3':
        name = input("Enter name to update contact : ")
        if name in contacts:
            age = input("Enter updated age : ")
            email = input("Enter updated email : ")
            mobile = input("Enter updated mobile : ")
            contacts[name] = {'age': int(age), 'email': email, 'mobile': mobile}
            print(f"Contact {name} updated successfully!")
            logging.info(f"\nContact updated:\n Name: {name}\n Age: {age}\n Email: {email}\n Mobile: {mobile}\n\n{'-'*60}")
        else:
            print('Contact not found!')
            logging.warning(f"\nAttempted to update non-existent contact:\n Name: {name}\n\n{'-'*60}")

    elif choice == '4':
        name = input("Enter contact name to delete : ")
        if name in contacts:
            del contacts[name]
            print(f"Contact name {name} has been deleted successfully!")
            logging.info(f"\nContact deleted:\n Name: {name}\n\n{'-'*60}")
        else:
            print('Contact not found!')
            logging.warning(f"\nAttempted to delete non-existent contact:\n Name: {name}\n\n{'-'*60}")

    elif choice == '5':
        search_name = input("Enter contact name to search : ")
        found = False
        for name, contact in contacts.items():
            if search_name.lower() in name.lower():
                print(f"Found - Name: {name}, Age: {contact['age']}, Mobile Number: {contact['mobile']}, Email: {contact['email']}")
                logging.info(f"\nSearch result: Found contact:\n Name: {name}\n Age: {contact['age']}\n Mobile: {contact['mobile']}\n Email: {contact['email']}\n\n{'-'*60}")
                found = True
        if not found:
            print("No contact found with that name")
            logging.info(f"\nSearch result: No contact found with name containing:\n {search_name}\n\n{'-'*60}")

    elif choice == '6':
        total_contacts = len(contacts)
        print(f"Total contacts in your book: {total_contacts}")
        logging.info(f"\nTotal contacts counted: {total_contacts}\n\n{'-'*60}")

    elif choice == '7':
        print('Good bye..Closing the program')
        logging.info(f"\nProgram exited\n\n{'-'*60}")
        break

    else:
        print("Invalid input!")
        logging.warning(f"\nInvalid input received\n\n{'-'*60}")
